...
def jac_score(y_pred, y_true):
    intersection = (y_true * y_pred).sum()
    union = y_true.sum() + y_pred.sum() - intersection
    return (intersection + 1e-15) / (union + 1e-15)


def sm_score(y_pred, y_true):
    sm = cal_sm()
    return sm.cal(y_pred, y_true)


def em_score(y_pred, y_true):
    em = cal_em()
    return em.cal(y_pred, y_true)


def wfm_score(y_pred, y_true):
    wfm = cal_wfm()
    return wfm.cal(y_pred, y_true)


def mae_socre(y_pred, y_true):
    mae = cal_mae()
...
def calculate_metrics(y_pred, y_true):
    ...
    score_smeasure = sm_score(y_p, y_t)
    score_fmeasure = wfm_score(y_p, y_t)
    score_mae = mae_socre(y_p, y_t)
    score_emeasure = em_score(y_p, y_t)
    ...
    ## Score
    score_dice = dice_score(y_pred, y_true)
    score_iou = iou_score(y_pred, y_true)
    score_f2 = F2(y_pred, y_true)
    score_recall = recall(y_pred, y_true)
    score_precision = precision(y_pred, y_true)
    score_acc = acc_score(y_pred, y_true)

    return [score_dice, score_iou, score_smeasure, score_emeasure, score_fmeasure, score_mae, score_precision, score_recall, score_f2]
